...
data <- st_read("data/nps_boundary/nps_boundary.shp") %>%
  filter(str_detect(PARKNAME, "Grand Teton")) %>%
  st_transform(crs = 3738)
...
hm %>%
  height_shade(texture = grDevices::colorRampPalette(colors, bias = 2)(256)) %>%
...
render_camera(phi = 90, zoom = .75, theta = 0)
...
add_stuff <- function(pal, map, c, c_fun, t, markups = TRUE) {
  ...
  # Crop out bottom 500 px
  img <- image_crop(img, geometry = "4241x5500")
  
  # Title
  img_ <- image_annotate(img, "Grand Teton", weight = 700, 
                         font = "Frijole", location = "+0+100",
                         color = text_color, size = 300, gravity = "north")
  
  # Subtitle
  img_ <- image_annotate(img_, "National Park", font = "Rock Salt",
  ...
  img_ <- image_annotate(img_, glue("Graphic by Spencer Schien (     @MrPecners) | ", 
                         ...
                         font = "Rock Salt", location = "+0+50",
                         color = alpha(text_color, .5), size = 52, gravity = "south")
...
add_stuff(pal = pal, map = map, c = 8, c_fun = colors, t = 6, markups = FALSE)
