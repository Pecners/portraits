...
data <- st_read("data/nps_boundary/nps_boundary.shp") %>%
  filter(str_detect(PARKNAME, "Shenandoah"))
...
render_camera(phi = 90, zoom = .6, theta = 0)
...
add_stuff <- function(pal, map, c, c_fun, t, markups = TRUE) {
  ...
  # Title
  img_ <- image_annotate(img, "Shenandoah", weight = 700, 
  ...
  if (markups) {
    ...
    prj <- "+proj=ortho +lat_0=30 +lon_0=-80"
...
add_stuff(pal = "flag", map = "shenandoah", c = 8, c_fun = colors, t = 1, markups = TRUE)
